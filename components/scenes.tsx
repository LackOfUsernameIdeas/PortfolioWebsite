...
function FloatingObject({ position, scale = 1, color, speed = 1, distort = 0.3 }: FloatingObjectProps) {
...
function TorusKnot({ position, color, scale = 1 }: { position: [number, number, number], color: string, scale?: number }) {
  const meshRef = useRef<THREE.Mesh>(null)
  ...
  return (
    <group ref={groupRef} position={[1.5, -0.5, 0]}>
      ...
      <mesh position={[0, 0.7, 0.03]}>
        ...
        <meshStandardMaterial color="#2563eb" emissive="#2563eb" emissiveIntensity={0.3} roughness={0.1} />
      </mesh>
      {/* Monitor stand */}
...
function MouseFollower() {
  ...
  return (
    <mesh ref={meshRef} position={[0, 0, 2]}>
      ...
      <meshStandardMaterial color="#f97316" emissive="#f97316" emissiveIntensity={0.5} transparent opacity={0.8} />
    </mesh>
  )
}
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      particles.forEach((particle, i) => {
        ...
        tempMatrix.scale(new THREE.Vector3(particle.scale, particle.scale, particle.scale))
...
export function HomeScene() {
  return (
    <Canvas camera={{ position: [0, 0, 6], fov: 50 }}>
      ...
      <Desk />
      <FloatingObject position={[-3, 2, -2]} scale={0.4} color="#f97316" speed={1.2} distort={0.4} />
      <FloatingObject position={[4, 1, -3]} scale={0.3} color="#3b82f6" speed={0.8} distort={0.3} />
      <FloatingObject position={[-2, -1.5, -1]} scale={0.25} color="#22c55e" speed={1.5} distort={0.5} />
      <MouseFollower />
    </Canvas>
  )
}
...
export function AboutScene() {
  return (
    <Canvas camera={{ position: [0, 0, 6], fov: 50 }}>
      ...
      {/* Grid lines */}
      <gridHelper args={[20, 40, '#3b82f6', '#1e3a8a']} position={[0, -2.5, 0]} />
      
      <TorusKnot position={[0, 0, 0]} color="#60a5fa" scale={0.6} />
      <FloatingObject position={[-3, 1, -2]} scale={0.3} color="#f97316" speed={1} distort={0.4} />
      <FloatingObject position={[3, -1, -2]} scale={0.25} color="#22c55e" speed={1.3} distort={0.3} />
...
export function ProjectsScene() {
  return (
    <Canvas camera={{ position: [0, 0, 8], fov: 50 }}>
      ...
      <FloatingObject position={[-4, 2, -3]} scale={0.3} color="#f97316" speed={1} />
      <FloatingObject position={[4, -2, -3]} scale={0.25} color="#3b82f6" speed={1.2} />
    </Canvas>
  )
}
...
export function ContactScene() {
  return (
    <Canvas camera={{ position: [0, 0, 6], fov: 50 }}>
      ...
      <FloatingObject position={[-3, 1, -2]} scale={0.4} color="#f97316" speed={1} distort={0.3} />
      <FloatingObject position={[-2, -1.5, -1]} scale={0.3} color="#3b82f6" speed={1.3} distort={0.4} />
      <FloatingObject position={[3, 2, -3]} scale={0.25} color="#22c55e" speed={0.9} distort={0.35} />
    </Canvas>
  )
}
