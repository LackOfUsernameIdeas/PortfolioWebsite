...
**Core features:**
- **Real-time EEG analysis:** The human brain produces bioelectrical signals — **brainwaves** — measurable via Electroencephalography (EEG). Using **Power Spectral Density (PSD)**, these are classified into five frequency bands: **Delta (0–4 Hz)** — deep relaxation or sleep; **Theta (4–7 Hz)** — creativity and intuition; **Alpha (8–12 Hz)** — calm and mentally coordinated; **Beta (12–30 Hz)** — active thinking and focus; **Gamma (30+ Hz)** — intense cognitive activity. Each band is further divided into sub-bands. NeuroSky's proprietary algorithms also extract two key scores (0–100): **Attention** and **Meditation**. The session runs ~1 minute, streaming live via **Bluetooth → ThinkGear Connector → PyMindWave2 → Socket.IO → Node.js API → React app**, where it is visualized in real time. After the session, aggregated data is passed to the AI to determine the appropriate content corrective for the user's cognitive state
- **AI recommendations** for films, TV series, books, and songs, generated via **OpenAI** and **Gemini APIs** using heavily engineered prompts. Model access is managed through **LangChain**, which abstracts LLM integration and enables structured, comparable testing across models. After extensive benchmarking — testing **Claude 3 Opus**, **Mistral Large**, **BgGPT-7B**, **Gemini 1.5 Pro**, and the OpenAI family — **GPT-4o** proved most reliable for films and series, and **GPT-4-Turbo** for books
- **Custom relevance algorithm** that determines whether a recommendation is relevant to a user's preferences across six criteria: preferred genres (2pts — highest weight), content type, user mood mapped to genres, available time vs. runtime, release year range, and target audience. A score above **5 points** is deemed relevant, producing the True/False Positive/Negative values that feed the ML evaluation suite
- **ML evaluation suite** measuring recommendation quality with 7 metrics: **Precision**, **Recall / TPR**, **F1 Score**, **Accuracy**, **Specificity / TNR**, **FPR**, and **FNR**. A separate per-generation Precision is also calculated for the 5 most recently generated recommendations. All backed by **86 unit tests** across user, recommendation, preference, statistics, and metrics functions
- **VR cinema experience** built with **A-Frame** (Three.js + WebGL), targeting **Oculus / Meta Quest 2**. The scene recreates a real cinema hall with projectors, a popcorn machine, and a large screen, where users can watch film trailers
- **Book data pipeline** combining a **custom Python/Beautiful Soup scraper** targeting Goodreads (whose public API was shut down in 2020) with **Google Books API** as supplementary enrichment
- **Song data pipeline** combining **Spotify API** (rich metadata) with **YouTube API** (view counts, likes, comment counts, and direct music video links), merged after the AI returns song titles
- **Scraped and enriched content metadata** from **IMDb, OMDb API, Goodreads, Google Books API, YouTube API, and Spotify API**, with platform-wide and per-user statistics rendered via **ApexCharts**
...
**Development notes:**
- Parts of the **PyMindWave2** library were incompatible with the project's requirements and had to be rewritten from scratch
- The VR cinema faced a core blocker: **A-Frame cannot render \`<iframe>\` elements**, making YouTube embeds impossible. The solution was downloading trailers as MP4s via **yt-dlp** — but running it on shared hosting was blocked (no sudo, Python 3.6.8 only). The final architecture: **yt-dlp runs inside a Dockerized Python service on Google Cloud Run**, downloading trailers and storing them in **Google Cloud Storage (GCS)**, served via public GCS URLs into the A-Frame \`<video>\` element. A-Frame's built-in hand controllers also required a separate custom fix — the official documentation's own embedded demo was broken
- **Goodreads** — the most comprehensive book database — shut down its public API in 2020. After exhausting every available alternative, the scraper solution extracts a large hidden JSON object embedded in a \`<script>\` tag on Goodreads pages, containing complete book metadata including fields not visible on the page itself
- The **Spotify API** frequently lacked data for older or less-popular tracks, which is why YouTube API was added as a complementary source. Early on, the AI regularly **hallucinated fictional songs** that existed on neither platform — resolved through extensive prompt iteration and testing across genre/preference combinations
- **Gemini** produced valid but overly generic justifications compared to GPT; the remaining models (Claude 3 Opus, Mistral Large, BgGPT-7B) were outperformed by the OpenAI family across all evaluation criteria`,
    technologies: [
      "React",
      "TypeScript",
      "Vite",
