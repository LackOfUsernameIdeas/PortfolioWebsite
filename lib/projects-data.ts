...
**Core features:**
- **Goal selection algorithm:** Critical health states always take highest priority — critically low BMI (moderate/severe undernutrition) or dangerously low body fat (below 2% for men, below 10% for women) unconditionally triggers **Dirty Bulk**; Obesity Class II/III triggers **Aggressive Cut**; Class I triggers **Cut**. Outside critical cases, the recommendation is determined by the interaction of BMI and BF% categories — for example, normal BMI with elevated BF% maps to **Recomposition**, while pre-obese BMI with athlete-level BF% (where excess weight is muscular) maps to **Maintenance**. Users retain full freedom not to follow the recommended goal and adjust macronutrient ratios based on individual needs, preferences, and health conditions, with medical or nutritional consultation advised for cases involving illness, pregnancy, or other health-related factors.
- **Macronutrient distribution** grounded in **WHO/EFSA** guidelines, with elevated protein ranges to support muscle synthesis, satiety, and recovery. Each goal applies a specific caloric offset from TDEE — Cut (–500 kcal), Aggressive Cut (–750 kcal), Lean Bulk (+300 kcal), Dirty Bulk (+500 kcal), Recomposition (–200 kcal), Aesthetic (–300 kcal), Strength (+200 kcal), Maintenance (±0 kcal). CDC and NHS guidance supports safe rates of ~0.5–1 kg/week loss or ~0.25–0.5 kg/week gain.
...
**Development notes:**
- After benchmarking multiple models, **GPT-5.2** proved the most reliable for generating relevant, logically connected fitness and nutrition recommendations — GPT-4o was initially preferred due to prior experience across other projects, but its output was not sufficiently relevant and its overall performance was not convincing enough for this use case.
- The body-proportional calibration tolerance system was a core design challenge: without converting absolute joint distances (in mm) into percentages relative to each user's body segment lengths, the system would incorrectly penalize tall users for deviations that are perfectly normal for their proportions while being too lenient with shorter users. The percentage-based approach solves this entirely.
- The angle tolerance gradient (\`max(0, 100 × (1 – diff / (2 × angle_tolerance)))\`) was a deliberate tradeoff — requiring exact angles would make exercises practically impossible to complete due to natural tremor, depth sensor noise, and skeleton tracking imprecision. The smooth scoring formula rewards precision while remaining achievable.
- The forecast engine accounts for **training frequency** as an additional factor — user-specified training days drive both pre/post-workout meal inclusion in the diet plan and the weight change projection, making the estimate closer to real-world outcomes than a purely caloric model.`,
    technologies: [
      "Next.js",
      "React",
      "TypeScript",
