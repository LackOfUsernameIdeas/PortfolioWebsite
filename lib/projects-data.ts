...
**Core features:**
- **Real-time EEG analysis:** The human brain produces bioelectrical signals — **brainwaves** — measurable via Electroencephalography (EEG). Using **Power Spectral Density (PSD)**, they are classified into five frequency bands: **Delta (0–4 Hz)** — deep relaxation or sleep; **Theta (4–7 Hz)** — creativity and intuition; **Alpha (8–12 Hz)** — calm and mentally coordinated; **Beta (12–30 Hz)** — active thinking and focus; **Gamma (30+ Hz)** — intense cognitive activity. There are also sub-bands whose characteristics correspond to those of their parent band. NeuroSky's proprietary algorithms also extract two key scores (0–100): **Attention** and **Meditation** level. The session runs 1 minute, streaming live data via **Bluetooth → ThinkGear Connector → PyMindWave2 → Socket.IO → Node.js API → React app**, where it is visualized in real time. After the session, all data is passed to the AI to determine the appropriate content as corrective for the user's cognitive state
...
- **JWT + HMAC/SHA-256 authentication**, GitHub branching strategy (main / dev / per-feature branches), Trello-managed task distribution throughout the development period, as well as 86 unit tests across user, recommendation, preference, statistics, and metrics functions
